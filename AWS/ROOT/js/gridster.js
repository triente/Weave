...
require([ 'jquery', 'jquery.gridster', 'jquery-ui', 'indicatorPanel' ],
		function($) {
			$(document).ready(
					$(function() { // DOM Ready

						goog.require('goog.string.StringBuffer');
						goog.require('aws.templates.indicatorPanel');
						goog.require('aws.templates.gridster');
						goog.require('goog.dom');
						goog.require('goog.dom.DomHelper');
						goog.require('aws.Panel');
						var gridList = {
							'grid' : [ {
								'row' : '1',
								'col' : '1',
						...
						soy.renderElement($('.gridster')[0], aws.templates.gridster, gridList);
						var data = {
							'panelType' : 'indicator',
						...
						/*soy.renderElement($('li[data-col="1"][data-row="2"]')[0],
								aws.templates.indicatorPanel, data);*/
						...
						scriptCombobox.append($("<option/>").val('script1').text("Script 1"));
						scriptCombobox.append($("<option/>").val('script2').text("Script 2"));

						var dataCombobox = $('#dataCombobox');

						dataCombobox.append($("<option/>").val('script1').text("Obesity.csv"));
						dataCombobox.append($("<option/>").val('script2').text("Cars.csv"));
			...
			function queryRService(method, params, callback, queryID) {
				...
				// resulttextarea.value = 'Awaiting Response for ' + method + '
				// request....';
			}

			// ---------------------------------called by scriptOneButton
			// BEGIN--------------------------
			...
			function scriptOneQuery(secondMethodName) {
				...
				function setCSV2Source(rResult) {
					...
					weave.path('MyNORMDataSource').request('CSVDataSource').vars({
			...
			function testServerQuery(secondMethodName) {
				...
				function handleRResult(response) {
					if (response.error) {
						...
						// console.log("retrieved " + rResult.length +
						// "results");
				...
				function setCSVSource(rResult) {
					 ...
					 * csvDataSource the source to use column from columnNumber
					 * columnIndex or columnName in the csvDatasource
					 */
					weave.setCSVColumn = function(columnPath, csvDataSource, columnNumber) {
						...
						var x = weave.path(csvDataSource);// gets the path
						// object
						x.vars({
							i : columnNumber,
							p : columnPath
						});// adds the required variables and attributes them
						var y = x.exec('putColumn(i,p)');// calling funtion
						// putColumn from
						// CSVDataSource.as
						// in Weave
					};

					weave.setCSVColumn([ 'SPT', 'children', 'visualization', 'plotManager',
							'plotters', 'plot', 'dataX' ], 'MyDataSource', 'PercentObese2002');
					weave.setCSVColumn([ 'SPT', 'children', 'visualization', 'plotManager',
							'plotters', 'plot', 'dataY' ], 'MyDataSource', 'PercentObese2004');

					// can also specify the columnIndex
					// weave.setCSVColumn(['SPT','children','visualization','plotManager','plotters','plot','dataY'],'MyDataSource',
					// 'PercentObese2004');

					// setting the color column, columnIndex is hardcoded
					weave.setCSVColumn([ 'defaultColorDataColumn', 'internalDynamicColumn' ],
							'MyDataSource', 0);

					// var scatterPlotX =
					// weave.path("SPT","children","undefinedX","plotManager","plotters","plot","dataX","internalObject","dynamicColumnReference","internalObject");
					// scatterPlotX.state("hierarchyPath",
					// '<hierarchy><attribute keyType="null"
					// csvColumn="PercentObese2002"
					// title="PercentObese2002"/></hierarchy>');

					// var scatterPlotX =
					// weave.path("SPT","children","undefinedXY","plotManager","plotters","plot");
					// scatterPlotX.state = ("dataX",
					// '<ReferencedColumn><dynamicColumnReference
					// encoding="dynamic">
					// <HierarchyColumnReference><dataSourceName>MyDataSource</dataSourceName><hierarchyPath
					// encoding="xml"><hierarchy><attribute
					// csvColumn="PercentObese2002" keyType="null"
					// title="PercentObese2002"/></hierarchy></hierarchyPath></HierarchyColumnReference></dynamicColumnReference></ReferencedColumn>');

					// var scatterPlotY =
					// weave.path("SPT","children","undefinedXY","plotManager","plotters","plot");
					// scatterPlotY.state = ("dataY",
					// '<ReferencedColumn><dynamicColumnReference
					// encoding="dynamic">
					// <HierarchyColumnReference><dataSourceName>MyDataSource</dataSourceName><hierarchyPath
					// encoding="xml"><hierarchy><attribute
					// csvColumn="PercentObese2004" keyType="null"
					// title="PercentObese2004"/></hierarchy></hierarchyPath></HierarchyColumnReference></dynamicColumnReference></ReferencedColumn>');
					// var scatterPlotY = weave.path("SPT", "children",
					// "undefinedY", "plotManager", "plotters", "plot", "dataY",
					// "internalObject", "dynamicColumnReference",
					// "internalObject");
					// scatterPlotY.state("hierarchyPath", '<hierarchy>
					// <attribute keyType="null" csvColumn="PercentObese2004"
					// title="PercentObese2004"/></hierarchy>');
